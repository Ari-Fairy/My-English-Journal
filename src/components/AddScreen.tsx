    ...
    if (!success) {
      ...
      try {
        ...
        if (extracted.length > 0) {
          setParsed(extracted);
          setReview(true);
          setMsg("✅ Слова распознаны с помощью встроенного сканера!");
        } else {
          setMsg("❌ Не удалось найти разборчивые слова на фото. Попробуйте сделать более чёткий и освещённый снимок.");
        }
      } catch (tessErr) {
        console.warn("Client Tesseract failed:", tessErr);
        setMsg("❌ Не удалось распознать фото. Попробуйте сделать более чёткий снимок.");
      }
    }
